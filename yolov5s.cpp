...
{
    FILE *fp;                     // 文件指针
    unsigned char* model_data;    // 模型数据缓冲区（malloc 分配）

    fp = fopen(model_path, "rb"); // 以二进制只读方式打开模型文件
    if(fp == NULL)
    {
        printf("open model failed!\n"); // 打开失败
        // 新手提示：这里最好 return nullptr; 并在外层判断，避免后续 fseek/ftell 崩溃
    }
    ...
    model_size = ftell(fp);           // ftell 获取当前文件指针偏移量，即文件大小（字节）
    
    model_data =(unsigned char *) malloc(model_size); // 分配足够内存存放模型文件
    // 新手提示：malloc 失败会返回 nullptr，最好检查一下
    ...
    ret = fread(model_data, 1, model_size, fp); // 读取整个文件到 model_data
    if(ret<0)
    {
        // 注意：fread 返回 size_t（无符号），严格来说不会 <0；通常用 ret != model_size 判断是否读满
        printf("read model failed! err: %d\n",ret);
    }

    // 新手提示：这里缺少 fclose(fp); 会导致文件描述符泄漏（建议补上）
    // fclose(fp);
