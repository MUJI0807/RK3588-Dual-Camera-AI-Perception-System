...
static AVPacket pkt;    // 全局AVPacket用于发送数据,AVPacket 是 FFmpeg 里“压缩后的媒体数据包”
...
{
        static int64_t pts = 0; // 显示时间戳
        static int64_t dts = 0; // 解码时间戳
        ...
        pkt.dts = dts;        // 设置 AVPacket 的解码时间戳
        pkt.duration = frame_duration;    // 设置 AVPacket 的持续时长
        pts += frame_duration;   // 更新下一个帧的显示时间戳
        dts += frame_duration;   // 更新下一个帧的解码时间戳
...
{       
        ...
        out_stream->time_base = av_make_q(1, config->fps);  // 设置时间基为1/帧率, config->fps是帧率
        ...
        o_codec_ctx->time_base = av_make_q(1, config->fps);  // 设置时间基为1/帧率
